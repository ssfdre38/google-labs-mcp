...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$zipUrl = 'https://www.gyan.dev/ffmpeg/builds/ffmpeg-release-essentials.zip'; ^
   $zipPath = Join-Path $env:TEMP 'ffmpeg_essentials.zip'; ^
   Write-Host 'Downloading FFmpeg release from' $zipUrl '...'; ^
   ...
   $found = Get-ChildItem -Path (Join-Path $env:TEMP 'ffmpeg_extracted') -Filter 'ffmpeg.exe' -Recurse | Select-Object -First 1; ^
   if ($found) { ^
     Copy-Item $found.FullName -Destination '%FFMPEG_EXE%' -Force; ^
     Write-Host 'Successfully installed portable FFmpeg to: %FFMPEG_EXE%'; ^
   } else { ^
     Write-Error 'Could not locate ffmpeg.exe in archive.'; ^
   } ^
